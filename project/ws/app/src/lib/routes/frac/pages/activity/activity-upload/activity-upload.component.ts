...
export class ActivityUploadComponent implements OnInit, OnDestroy {
  ...
  onSaveClicked(): void {
    ...
    const payloads = changedRows
      .map(row => {
        ...
        return FracPayloadBuilder.buildGenericUpdate('Activity', row, original, languageCode)
      })
  ...
  onRemoveClicked(): void {
    ...
    confirmationDialogRef.afterClosed().pipe(takeUntil(this.destroy$)).subscribe((action: 'continue' | 'cancel' | undefined) => {
      ...
      const deletePayload = this.selectedRows
        .map(row => FracPayloadBuilder.buildDelete('Activity', row, this.selectedLanguage))
        .filter(Boolean) as any[]
  ...
  onDownloadTemplate(): void {
    const fileUrl = getFracSampleTemplateUrl('activity', this.selectedLanguage)
